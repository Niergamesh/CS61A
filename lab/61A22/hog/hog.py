...
def hefty_hogs(player_score, opponent_score):
    ...
    while opponent_score:
        hefty_hogs_digit=opponent_score%10
        player_score=digit_fn(hefty_hogs_digit)(player_score)
        opponent_score //=10
...
def hog_pile(player_score, opponent_score):
    ...
    if player_score%10==opponent_score%10:
        hog_pile_digit=opponent_score%10
        return hog_pile_digit
    return 0
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0<goal and score1<goal:
        if who==0:
            ...
            turn0=take_turn(roll0, score0, score1, dice, goal)
            score0+=turn0
            if score0%10==score1%10:
                hog_pile0=hog_pile(score0,score1)
                score0+=hog_pile0
        if who==1:
            roll1=strategy0(score1,score0)
            turn1=take_turn(roll1, score1, score0, dice, goal)
            score1+=turn1
            if score0%10==score1%10:
                hog_pile1=hog_pile(score0,score1)
                score1+=hog_pile1
        who=next_player(who)
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    max_average=average_roll(num_dice_max,dice)

    while num_dice<=10:
        average=average_roll(num_dice,dice)
        if average>max_average:
            num_dice_max=num_dice
            max_average=average
        num_dice+=1
    return num_dice_max
